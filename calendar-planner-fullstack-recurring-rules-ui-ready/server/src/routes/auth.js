...
const googleScopes = (process.env.GOOGLE_SCOPES || 'https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/calendar').split(' ');
...
router.get('/google', passport.initialize(), passport.authenticate('google', { scope: googleScopes, accessType: 'offline', prompt: 'consent' }));

router.get('/google/callback', passport.initialize(), passport.authenticate('google', { failureRedirect: '/' }), async (req, res) => {
  const user = req.user;
  const token = issueToken(user);
  res.cookie(COOKIE_NAME, token, {
    httpOnly: true,
